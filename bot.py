...
# Формула расчета честного коэффициента для игры Мины (с учетом открытых ячеек)
def get_mines_multiplier(total_mines, opened_count):
    ...
    try:
        ...
        if ways_safe == 0:
            return 0
        return round((ways_total / ways_safe) * 0.96, 2) # 4% комиссия казино для баланса
    except Exception:
        return 1.0
...
def render_tower_text(current_level, bet, mines_count, next_win, current_win=0):
    ...
    for lvl in range(5, 0, -1):
        if lvl > current_level:
            rows.append(f"<b>Этаж {lvl}: ⬜ ⬜ ⬜ ⬜ ⬜</b>")
        elif lvl == current_level:
            rows.append(f"<b>Этаж {lvl}: ❓ ❓ ❓ ❓ ❓  ◀️</b>")
        else:
            rows.append(f"<b>Этаж {lvl}: ✅ ✅ ✅ ✅ ✅ (Пройден)</b>")
    return (
        f"🏰 <b>ИГРА: БАШНЯ</b>\n\n" + "\n".join(rows) + "\n\n"
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        f"👉 <b>МИНЫ [ставка] [мины]</b> — <b>Новая игра 5х5 (от 1 до 24 мин)</b>"
    )

# Ежедневный Бонус (от 300 до 10000)
...
async def get_daily_bonus(message: types.Message):
    ...
    if current_time - user['last_bonus'] < cooldown:
        time_passed = current_time - user['last_bonus']
        time_left = cooldown - time_passed
        hours = time_left // 3600
        minutes = (time_left % 3600) // 60
        await message.answer(f"<b>⏳ ВЫ УЖЕ ЗАБИРАЛИ БОНУС!</b>\n<b>Приходите снова через: {hours} ч. и {minutes} мин.</b>")
        return
    ...
    await message.answer(
        ...
        f"<b>💰 Текущий баланс:</b> <b>{user['balance'] + bonus_amount} Ucoin</b>"
    )
...
async def start_mines(message: types.Message, state: FSMContext):
    if await state.get_state() in [GameStates.playing_tower, GameStates.playing_mines]:
        await message.answer("<b>❌ У вас уже открыта игровая сессия! Завершите текущую игру.</b>")
        return

    parts = message.text.split()
    if len(parts) != 3:
        await message.answer("<b>⚠️ НЕВЕРНЫЙ ФОРМАТ!</b>\nИспользуйте: <code>Мины [ставка] [мины]</code>\nПример: <code>Мины 250 5</code>")
        return

    try:
        bet, mines_count = int(parts[1]), int(parts[2])
    except ValueError:
        await message.answer("<b>❌ Ставка и мины должны быть числами!</b>")
        return

    if bet <= 0 or not (1 <= mines_count <= 24):
        await message.answer("<b>❌ Ограничения: Ставка больше 0, а количество мин строго от 1 до 24!</b>")
        return

    user = await database.get_user(message.from_user.id, message.from_user.full_name)
    if user['balance'] < bet:
        await message.answer(f"<b>❌ Недостаточно средств! Ваш баланс: {user['balance']} Ucoin</b>")
        return

    await database.start_game_bet(message.from_user.id, bet)

    # Создаем поле 25 ячеек (True = Мина)
    grid = [False] * 25
    ...
    for i in range(25):
        builder.button(text="⬛", callback_data=f"mn_clk_{i}")
    builder.adjust(5) # Сетка 5х5 инлайн

    await message.answer(
        f"<b>💣 ИГРА: МИНЫ (Поле 5х5)</b>\n\n"
        f"💵 <b>Ставка:</b> <b>{bet} Ucoin</b>\n"
        f"💥 <b>Всего мин на поле:</b> <b>{mines_count}</b>\n"
        f"💎 <b>Открыто алмазов:</b> <b>0 / {25 - mines_count}</b>\n"
        f"📈 <b>Текущий множитель:</b> <b>1.0х (0 Ucoin)</b>",
...
async def process_mines_click(callback: types.CallbackQuery, state: FSMContext):
    ...
    if grid[cell_idx]:
        ...
        await callback.message.edit_text(
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📉 <b>Проиграно:</b> <b>{bet} Ucoin</b>",
            reply_markup=builder.as_markup()
        )
        return

    # Успешно открыт алмаз
    opened_count += 1
    ...
    # Если открыты ВСЕ чистые клетки — автоматическая победа!
    if opened_count == (25 - mines_count):
        ...
        await callback.message.edit_text(
            f"<b>🏆 ВЫ СОВЕРШИЛИ НЕВОЗМОЖНОЕ И ОЧИСТИЛИ ПОЛЕ!</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎉 <b>Ваш джекпот:</b> <b>{current_win} Ucoin ({mult}x)!</b>",
            reply_markup=builder.as_markup()
        )
        return
    ...
    for i in range(25):
        ...
        builder.button(text=text, callback_data=f"mn_clk_{i}")
    
    # Кнопка ручного кэшаута
...
async def process_mines_cashout(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"<b>💰 ВЫ СВОЕВРЕМЕННО ЗАБРАЛИ ДЕНЬГИ!</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💵 <b>Выигрыш:</b> <b>{current_win} Ucoin ({mult}x) зачислен!</b>",
        reply_markup=None
    )

# --- ОСТАЛЬНЫЕ ОБРАБОТЧИКИ (БАШНЯ) ---
...
async def start_tower(message: types.Message, state: FSMContext):
    if await state.get_state() in [GameStates.playing_tower, GameStates.playing_mines]:
        await message.answer("<b>❌ У вас уже запущена игра! Завершите её перед новой.</b>")
        return
    ...
    if bet <= 0 or not (1 <= mines_count <= 4):
        await message.answer("<b>❌ Мин в башне должно быть от 1 до 4!</b>")
        return
...
async def tower_turn(callback: types.CallbackQuery, state: FSMContext):
    ...
    if current_mines[chosen_cell]:
        await database.lose_game(callback.from_user.id, bet)
        await state.clear()
        await callback.message.edit_text(f"<b>💥 БАБАХ! НА {current_level}-М ЭТАЖЕ ОКАЗАЛАСЬ МИНА!</b>\n<b>📉 Минус {bet} Ucoin.</b>", reply_markup=None)
        return
    ...
    if current_level == 5:
        await database.win_game(callback.from_user.id, current_win)
        await state.clear()
        await callback.message.edit_text(f"<b>🏆 ВЫ СНЕСЛИ БАШНЮ! Полный выигрыш: {current_win} Ucoin!</b>", reply_markup=None)
        return
...
async def tower_cashout(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"<b>💰 ВЫ ЗАБРАЛИ С БАШНИ {data.get('accumulated_win', 0)} UCOIN!</b>", reply_markup=None)
    await state.clear()
